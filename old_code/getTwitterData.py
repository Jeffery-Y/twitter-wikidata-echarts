...
def getTwitterData(input_screen_name = 'realDonaldTrump',relation_level = 3):
    ...
    def getUserShowLimitTime():
        local_time = int(time.time())
        limit_content = t.application.rate_limit_status()
        limit_timeline = limit_content.get('resources').get('users').get('/users/show/:id')
        return (limit_timeline.get('remaining'), (limit_timeline.get('reset') - local_time))
    ...
    while True:
        try:
            user_show = t.users.show(screen_name=user_name, include_entities=False)
            break
        except Exception as e:
            print('get user_show ERROR:', e)
    ...
    for P, Q in user_wikidata.items():
        Q_len = len(Q)
        Q_list = list(Q)
        line_num = int(Q_len/PROPERTY_LENGTH_EACHRAW)
        for line in range(line_num, 0, -1):
            Q_list.insert(PROPERTY_LENGTH_EACHRAW*line, '<br>    ')
        Q = "".join(Q_list)
        user_wikidata_list.append(P + ': ' + Q)
    user_wikidata_str = reduce(lambda x, y: x + '<br>' + y, user_wikidata_list)
    friends_dic_nodes[user_name]['tooltip'] = user_wikidata_str
